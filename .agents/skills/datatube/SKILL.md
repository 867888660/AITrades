...
description: >
  ...
  research Polymarket/Binance markets, analyze news or event risk, inspect
  EventGraph data, validate/submit EventGraph change requests, create strategy
  drafts, run risk checks or simulations, create or analyze backtests, run batch
  backtests, optimize backtest parameters, inspect backtest history, import a
  backtest into the strategy workspace, review approvals, inspect strategy
  status, or audit agent runs.
  Trigger examples include "install DataTube",
  "start DataTube", "research BTC Polymarket markets",
  "test EventGraph capability", "create a strategy draft",
  "run a BTC backtest", "compare backtest batches", "review pending approvals",
  "DataTube status", "Polymarket research", and "Binance market research".
---
...
- **Backtest**: create or inspect backtest cases, run single or batch historical
  replays, wait for runs, import runs into the workspace, compare results,
  optimize parameters, or analyze Strategy Metrics and State Lanes. Use
