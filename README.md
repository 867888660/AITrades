...
后端计算采用“Web 控制面 + 持久队列 + 资源准入 + 隔离 worker”架构；Research 与
历史回测不会在 HTTP 请求内同步执行。故障隔离、队列进度和运维参数见
[后端分发与故障隔离](docs/BACKEND_DISPATCH_ARCHITECTURE.md)。
[Backend Architecture v2：智能路由与自动资源调度](docs/BACKEND_ARCHITECTURE_V2.md)。
